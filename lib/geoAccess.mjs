...
const TRUSTED_CRAWLER_PATTERN = /\b(?:Googlebot|Googlebot-Image|Googlebot-Video|Googlebot-News|GoogleOther|GoogleOther-Image|GoogleOther-Video|Google-InspectionTool|Google-Extended|Google-CloudVertexBot|Bingbot|Applebot|Applebot-Extended|OAI-SearchBot|ChatGPT-User|GPTBot|ClaudeBot|Claude-SearchBot|Claude-User|PerplexityBot|CCBot)\b/i;
...
export function isTrustedCrawlerUserAgent(userAgent) {
  return TRUSTED_CRAWLER_PATTERN.test(cleanText(userAgent));
}

function isCrawlerPublicPath(path) {
  return PUBLIC_CRAWLER_PATHS.has(path);
}

export function shouldBypassGeoAccess(pathname, userAgent = '') {
  const path = pathname || '/';
  return path.startsWith('/_next/') ||
    ...
    (isTrustedCrawlerUserAgent(userAgent) && isCrawlerPublicPath(path)) ||
...
function blockMessage(decision) {
  ...
  return 'We could not confirm that your visit is coming from California. To protect the California-specific calculator, we paused access for now.';
}
      ...
      <section class="panel" aria-labelledby="geo-title">
        ...
        <div class="content">
          ...
          <div class="actions">
            ...
            <a class="secondary" href="mailto:contact@californiasettlementcalculator.com?subject=California%20access%20help">Contact support</a>
          </div>
