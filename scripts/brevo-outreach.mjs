...
const FEELER_MAILING_ADDRESS_LINES = [
  'Katelyn Nguyen',
...
Mailing address:
${FEELER_MAILING_ADDRESS_LINES.join('\n')}

If this is not relevant, just reply "not interested" and I won't follow up.`;

  const htmlContent = `<!doctype html>
<html><body style="margin:0;padding:0;">
<div style="margin:0;padding:0;font-family:Arial,sans-serif;font-size:14px;line-height:1.45;color:#111;">
...
<p style="margin:0 0 14px 0;">It's a California auto-injury settlement calculator that actually gives users an educational estimate instead of stopping at a vague "contact a lawyer" screen. The estimate is built from practical California injury-value factors, kept intentionally conservative, and the site makes clear throughout that it is educational only, not legal advice or a guaranteed outcome.</p>
<p style="margin:0 0 14px 0;">The site had 1,000+ Cloudflare-tracked visitors in the last 30 days before a recent mobile/desktop and SEO rebuild. In other words, that traffic came before the improved version went live.</p>
<p style="margin:0 0 14px 0;">I'm planning to test this with one California PI sponsor rather than rotate a marketplace of firms. If the fit is right, that sponsor would be the named firm for eligible, consented California inquiries generated by the calculator during the test.</p>
<p style="margin:0 0 14px 0;">The important part is the filtering. This is not meant to be raw form-fill traffic. The flow is built around California geo eligibility, required accident/county details, a current-attorney screen so already-represented visitors are not routed as sponsor inquiries, a minimum completion-time gate, affirmative consent to share with the named sponsor, US mobile validation, SMS verification, and duplicate-phone suppression within 30 days.</p>
<p style="margin:0 0 14px 0;">I'm reaching out to a short list of California PI firms to see whether a flat monthly sponsorship or joint-ad placement is worth testing. This would not be a referral fee, pay-per-signed-client deal, or guarantee of volume, case value, conversion rate, or outcome. Any placement would need written terms, attorney/ethics review, and clear SB 37-aware disclosures.</p>
<p style="margin:0 0 14px 0;">Worth reviewing?</p>
<p style="margin:0 0 14px 0;">If so, reply "send details" and I'll send the traffic snapshot and a simple sponsorship outline.</p>
<p style="margin:0 0 14px 0;">Best,<br>Katelyn Nguyen<br><a href="mailto:${FEELER_FROM.email}">${FEELER_FROM.email}</a><br><a href="${FEELER_URL}">${FEELER_URL}</a></p>
<p style="margin:0 0 14px 0;">Mailing address:<br>${FEELER_MAILING_ADDRESS_LINES.map(escapeHtml).join('<br>')}</p>
<p style="margin:0;">If this is not relevant, just reply "not interested" and I won't follow up.</p>
</div>
</body></html>`.trim();
